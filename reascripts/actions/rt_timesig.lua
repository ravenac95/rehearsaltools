-- actions/rt_timesig.lua
-- Registered as a REAPER custom action, invoked via /rt/timesig OSC.
...
local handler_mod = dofile(repo_root .. "src/handlers/timesig.lua")
local adapter     = dofile(repo_root .. "src/reaper_api.lua")

local data, err = payload_mod.read()
if err then
  reaper.ShowConsoleMsg("[rt_timesig] payload error: " .. err .. "\n")
  return
end
...
if h_err then
  reaper.ShowConsoleMsg("[rt_timesig] handler error: " .. h_err .. "\n")
end
