-- actions/rt_tempo.lua
-- Registered as a REAPER custom action, invoked via /rt/tempo OSC.
...
local handler_mod = dofile(repo_root .. "src/handlers/tempo.lua")
local adapter     = dofile(repo_root .. "src/reaper_api.lua")

local data, err = payload_mod.read()
if err then
  reaper.ShowConsoleMsg("[rt_tempo] payload error: " .. err .. "\n")
  return
end
...
if h_err then
  reaper.ShowConsoleMsg("[rt_tempo] handler error: " .. h_err .. "\n")
end
