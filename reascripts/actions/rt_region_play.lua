-- actions/rt_region_play.lua
-- Registered as a REAPER custom action, invoked via /rt/region/play OSC.
...
if err then
  reaper.ShowConsoleMsg("[rt_region_play] payload error: " .. err .. "\n")
  return
end

local handler = handler_mod.new(adapter)
local _, h_err = handler.play(data)
if h_err then
  reaper.ShowConsoleMsg("[rt_region_play] handler error: " .. h_err .. "\n")
end
